...
import os
from tainter_model_v5 import TainterModel
...
config_file_name = "baseline_config_5"
...
def tainters_rhs(state, time, parameters):
    return tm.run_model(state, time, parameters)
...
df = pd.DataFrame(solution, columns=["Natural_Resources", "State_Inputs", "State_Capacity", "Bureaucracy", "Complexity", "Population"])
df["time"] = t
df = df[["time", "Natural_Resources", "State_Inputs", "State_Capacity", "Bureaucracy", "Complexity", "Population"]]
